FROM accetto/xubuntu-vnc-novnc:22.04
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ca-certificates gnupg curl wget software-properties-common apt-transport-https \
    xfce4-settings lightdm lightdm-gtk-greeter arc-theme papirus-icon-theme \
    pulseaudio alsa-utils vim-tiny htop git sudo bash-completion \
    fonts-firacode fonts-powerline fonts-font-awesome xclip xsel fonts-dejavu xfce4-goodies \
    && rm -rf /var/lib/apt/lists/*

RUN curl -fsSLo /usr/share/keyrings/brave-browser-archive-keyring.gpg \
    https://brave-browser-apt-release.s3.brave.com/brave-browser-archive-keyring.gpg \
    && echo "deb [signed-by=/usr/share/keyrings/brave-browser-archive-keyring.gpg] \
    https://brave-browser-apt-release.s3.brave.com/ stable main" > /etc/apt/sources.list.d/brave-browser.list \
...
RUN xfconf-query -c xfce4-desktop -p /backdrop/screen0/monitor0/image-path -s /usr/share/backgrounds/custom_wallpaper.jpg || true
